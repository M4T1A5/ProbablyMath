...
namespace pmath
{
    ...
    template<typename T>
    Vector3<T>::Vector3(const Vector3<T>& vector3)
        : x(vector3.x),
          y(vector3.y),
          z(vector3.z)
    { }
    ...
    Vector3<T>::Vector3(const Vector3<T2>& vector3)
    ...
    // Public
    template<typename T>
    Vector3<T>& Vector3<T>::scale(const Vector3<T>& scaleVector)
    {
    ...
    template<typename T>
    Vector3<T>& Vector3<T>::scale(const T& xScale, const T& yScale, const T& zScale)
    {
        this->x *= xScale;
        this->y *= yScale;
        this->z *= zScale;
    ...
    template<typename T>
    Vector3<T>& Vector3<T>::scale(const T& s)
    {
        return scale(s, s, s);
    }
    ...
    template<typename T>
    T Vector3<T>::distance(const Vector3<T>& vec1, const Vector3<T>& vec2)
    {
    ...
    template<typename T>
    Vector3<T> Vector3<T>::lerp(const Vector3<T>& vec1, const Vector3<T>& vec2, const T& t)
    {
        return (1 - t) * vec1 + t * vec2;
    }

    template<typename T>
    std::string pmath::Vector3<T>::toString() const
    {
    ...
    template<typename T>
    bool Vector3<T>::operator ==(const Vector3<T>& right) const
    {
    ...
    template<typename T>
    const T& Vector3<T>::operator [](const unsigned int index) const
    {
        assert(index < COMPONENTS);
        return (&x)[index];
    }
